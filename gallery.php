<!DOCTYPE html>
<html lang="pl">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Gallery-PDR PRIMO</title>
    <link rel="stylesheet" href="style.css"/>
    <link rel="stylesheet" href="twentytwenty.css"/>
    <link rel="stylesheet" href="ReturnUp.css">
    <script src="ReturnUp.js"></script>
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <script src="jquery.event.move.js"></script>
    <script src="jquery.twentytwenty.js"></script>
    <script src="jquery.js"></script>

    


</head>
<body>

    <?php include 'header_nav.php'; ?>
    <h3>Portfolio</h3>

    <div class="twenty-twenty-container">
        <img src="./Gallery/z1przed.png" alt="Zdjecie 1 przed" />
        <img src="./Gallery/z1po.png" alt="Zdjecie 1 po" /> 
    </div>
    
    <div class="twenty-twenty-container">
        <img src="./Gallery/z2przed.png" alt="Zdjecie 2 przed" />
        <img src="./Gallery/z2po.png" alt="Zdjecie 2 po" />
    </div>
    
    <div class="twenty-twenty-container">
        <img src="./Gallery/z3przed.png" alt="Zdjecie 3 przed"/>
        <img src="./Gallery/z3po.png" alt="Zdjecie 3 po" /> 
    </div>
    
    <button id="scrollToTopBtn" onclick="scrollToTop()">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#ffffff" viewBox="0 0 256 256">
            <path d="M208,80v88a64,64,0,0,1-128,0V51.31L45.66,85.66A8,8,0,0,1,34.34,74.34l48-48a8,8,0,0,1,11.32,0l48,48a8,8,0,0,1-11.32,11.32L96,51.31V168a48,48,0,0,0,96,0V80a8,8,0,0,1,16,0Z"></path>
        </svg>
    </button>
    
    <footer>
        <p>&copy; 2024 PDR PRIMO. Wszelkie prawa zastrzeżone.</p>
    </footer>
    
    <script>
        $(document).ready(function() {
            $(".twenty-twenty-container").twentytwenty(); // Inicjalizacja wtyczki
        });
    </script>

    

</body>
</html>
